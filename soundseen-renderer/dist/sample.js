...
function syntheticSpec(durationSeconds, scene) {
    const beats = [];
    const phrases = [];
    const onsets = [];
    ...
    return {
        ...
        frames_track: {
            ...
            centroid_norm: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, (_, i) => 0.3 + (i / Math.max(1, durationSeconds * 10 - 1)) * 0.4),
            harmonic_ratio: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, () => 1 - profile.angularity),
            chroma_strength: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, () => valence > 0.5 ? 0.65 : 0.35),
            rolloff: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, () => 0.5),
            zcr: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, () => profile.angularity * 0.6),
            spectral_contrast: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, () => 0.5),
            pitch_class: Array.from({ length: Math.ceil(durationSeconds / 0.10) }, (_, i) => valence > 0.5 ? (i * 7) % 12 : -1),
        },
    };
}
function serenfBlendBiomeWeights(v, a) {
