...
`;
export class MelancholicRainScene {
    object3D;
    camera;
    bgMaterial;
    rainPositions;
    ...
    constructor() {
        ...
        this.rainGeometry.setAttribute("aInstancePos", new THREE.InstancedBufferAttribute(this.rainPositions, 3));
        this.rainGeometry.setAttribute("aOpacity", new THREE.InstancedBufferAttribute(this.rainOpacities, 1));
    ...
    render(spec, ctx) {
        ...
        const liveCount = Math.min(MelancholicRainScene.RAIN_POOL, 400 + Math.round(ctx.buildIntensity * 1200));
        const vScale = 1.0 + ctx.audio.spectralFlux * 0.6 + ctx.audio.onsetStrengthEnv * 0.8;
        ...
        const posAttr = this.rainGeometry.getAttribute("aInstancePos");
        const dt = 1 / 60;
