...
async function renderVideoTrack(spec, page, videoOnlyPath, maxSeconds) {
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
        const t = frame / FPS;
        await page.evaluate((time) => window.__renderFrameAt(time), t);
...
export async function renderComposition(opts) {
    ...
    const maxSeconds = Math.min(opts.maxSeconds ?? Number.POSITIVE_INFINITY, spec.duration_seconds);
    const { browser, page } = await launchPage();
    try {
        const hostHtml = pathToFileURL(path.join(__dirname, "page", "host.html")).toString();
        await page.goto(hostHtml);
        await page.waitForFunction(() => window.__ready === true);
        await page.evaluate((s) => window.__loadSpec(s), spec);
